...
function ProviderDashboard() {
  ...
  const documents = useMemo(() => {
    const backendDocuments = Array.isArray(profile?.documents) ? (profile?.documents as Array<Record<string, unknown>>) : [];
    if (backendDocuments.length > 0) {
      return backendDocuments.map((doc, index) => ({
  ...
  const me = {
    id: String((profile?._id as string | undefined) ?? `PRO-${(user?._id ?? "0000").slice(-4)}`),
    name: user?.name ?? "Professional",
    status: String((profile?.status as string | undefined) ?? "draft"),
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            ...
            <dl className="grid gap-3 sm:grid-cols-2">
              {[
                ...
                { label: "Applied on", value: profile?.createdAt ? new Date(profile.createdAt as string).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }) : "Today" },
